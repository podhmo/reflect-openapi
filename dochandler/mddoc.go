...
//  import {Prism as SyntaxHighlighter} from 'https://esm.sh/react-syntax-highlighter@15.5.0?bundle'
  ...
  const HeadingRenderer = props => {
	...
	const slug = text.toLowerCase().replace(/[{\/\.}]+/g, '').replace(/[ \t]+/g, '-'); // see: docgen.toHtmlID()

	const a = React.createElement('a', {"class": "x-anchor", "aria-hidden": "true", href: "#"+slug }, props.children);
	return React.createElement('h' + props.level, { id: slug, tabindex: "-1", dir: "auto" }, a);
  };

  const text = document.getElementById("mdtext").innerText; // innerHTML?

  const domNode = document.getElementById("mdbody");
  const root = createRoot(domNode);
  root.render(ReactMarkdown({children:text,  components:{'h1': HeadingRenderer, 'h2': HeadingRenderer, 'h3': HeadingRenderer}, remarkPlugins:[remarkGfm]}));
